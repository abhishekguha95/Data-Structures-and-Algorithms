...
function naiveSolutionArraySquared(arr1, arr2) {
  ...
  for (let i of arr1) {
    ...
    if (arr2_index === -1) {                // thus making this nested loop : O(N2)
...
function optimalSolutionArraySquared(arr1, arr2) {
  if (arr1.length !== arr2.length) {   // initial length check
    return false;
  }
  let arr1_obj = {};   // using objects 
  let arr2_obj = {};

  for (let i of arr1) {                       // key is array element, value is frequency
